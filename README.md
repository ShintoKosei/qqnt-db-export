...
需要准备 SQLCipher CLI。可以把 `sqlcipher.exe` 放进 `PATH`，也可以用 `--sqlcipher` 指定真实路径。Windows 可用版本可从 QQBackup 的 SQLCipher 构建下载：
...
```powershell
python scripts\qqnt_windows_export.py --kill-qq-first --account YOUR_QQ_NUMBER --outdir RE\windows_qq_export --sqlcipher tools\sqlcipher\sqlcipher-x64.exe
```
...
```powershell
python scripts\qqnt_windows_export.py --kill-qq-first --account YOUR_QQ_NUMBER --outdir RE\windows_qq_export --sqlcipher tools\sqlcipher\sqlcipher-x64.exe --no-extract
```
