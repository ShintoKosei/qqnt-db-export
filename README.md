...
8. 调用 SQLCipher 导出明文 SQLite 数据库。
9. 如果提供了 `--account`，自动提取本人消息。

需要准备 SQLCipher CLI。可以把 `sqlcipher.exe` 放进 `PATH`，也可以用 `--sqlcipher` 指定路径。Windows 可用版本可从 QQBackup 的 SQLCipher 构建下载：
...
```powershell
python scripts\qqnt_windows_export.py --kill-qq-first --account YOUR_QQ_NUMBER --outdir RE\windows_qq_export --sqlcipher C:\path\to\sqlcipher.exe
```
...
```powershell
python scripts\qqnt_windows_export.py --kill-qq-first --account YOUR_QQ_NUMBER --outdir RE\windows_qq_export --sqlcipher C:\path\to\sqlcipher.exe --no-extract
```
...
- **Windows 提示找不到 SQLCipher**：下载 SQLCipher CLI，或用 `--sqlcipher C:\path\to\sqlcipher.exe` 指定路径。
...
- `scripts/qqnt_windows_export.py`: Windows QQ NT key capture, database copy, SQLCipher decrypt, and self-message extraction helper.
